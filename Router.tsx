...
function match(realPath: string[], definedPath: string[]): boolean {
...
function extractPathParam(realPath: string[], definedPath: string[]) {
...
export function createRouter(store: { path: string }, strategy: PathResolutionStrategy = PathStrategy) {
  const Router: Component<{
    ...
    const [url, Page] = Object.entries(routes)
      ...
      .find(([url]) => match(path, url)) ?? [[], () => <h2>404 Not found</h2>];
...
export const { Router, onRouteChange, href, Link } = createRouter(store);
